...
namespace chess
{
    public class PieceRook : PiecesGeneral
    {
        public PieceRook() : base() { }
        ...
        public string[,] CreateValidMoveArray(string piece, string location)
        {
            ...
            for (int i = 1; i <= 77; i++)
            {
                ...
                if (line1.IsPointOnLine(p2) || line2.IsPointOnLine(p2))
                {
                    switch (p2String[0])
                    {
                            ...
                            if (p1.Y == borderLocation.Y)
                            {
                                ...
                                if (j > p1.X)
                                {
                                    for (int t = j; t < ValidLocations.GetLength(1); t++)
                                    {
                                        ...
                                        {
                                            //  Console.WriteLine("equal queen: " + "    locations: " + PieceLocations.BoardLocations[l, t]);
                                            //  Console.Write(" : on line -");
                                            //  Console.WriteLine("\n         p1 X: " + p1.X + "         borderLocation X: " + borderLocation.X + "         testLocation X: " + testLocation.X);
                                            if (line2.IsPointBad(p1, borderLocation, testLocation))
                                            {
                                                // Console.WriteLine("        bad on line2 -: nulled");
                                    ...
                                    for (int t = j; t < ValidLocations.GetLength(1); t++)
                                    {
                                        ...
                                        if (borderLocation.Y > testLocation.Y)
                                        {
                                            ...
                                            if (p1.X == borderLocation.X)
                                            {
                                                ...
                                                // Console.WriteLine("p1 y: " + p1.Y + "         borderLocation X: " + borderLocation.Y + "         testLocation X: " + testLocation.Y);
                                                if (line1.IsPointBad(p1, borderLocation, testLocation))
                                                {
                                                    // Console.WriteLine("        bad on line1 |: nulled");
                                                    // Console.Write("before: " + ValidLocations[l, t]);
                                                    ValidLocations[l, t] = "null";
                                                    // Console.Write("         after:" + ValidLocations[l, t] + "\n");
                                                    continue;
                                ...
                                for (int l = i; l >= 0; l--)
                                {
                                    for (int t = 7; t >= 0; t--)
                                    {
                                        ...
                                        if (borderLocation.Y < testLocation.Y)
                                        {
                                            ...
                                            {
                                                // Console.Write(" : on line |");
                                                // Console.WriteLine("\n         p1 y: " + p1.Y + "         borderLocation Y: " + borderLocation.Y + "         testLocation Y: " + testLocation.Y);
                                                if (line1.IsPointBad(p1, borderLocation, testLocation))
                                                {
                                                    // Console.Write("        bad on line1 |: nulled");
                                                    // Console.Write("before: " + ValidLocations[l, t]);
                                                    ValidLocations[l, t] = "null";
                                                    // Console.Write("         after:" + ValidLocations[l, t] + "\n");
                                                    continue;
